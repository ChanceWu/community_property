...
class EquallySharedCost extends React.Component {
	constructor(props) {
		super(props)
		this.state = {
			costList: ''
	...
	getCostList = () => {
		this.props.getCostList('equallyshared').then(()=>{
			this.setState({
	...
	addCost = (values) => {
		const data = Object.assign({}, values, {charge_type: 'equallyshared'})
	...
	handleSearch = (value) => {
		this.props.getCostList('equallyshared', value).then(()=>{
			this.setState({
				costList: this.props.cost
			})
		})
...
export default EquallySharedCost